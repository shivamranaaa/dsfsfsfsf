...
static const int PERSON_CLASS_ID = 0;  // Assuming '0' is the ID for 'person'
        ...
        for (int32_t grid_x = 0; grid_x < grid_w; grid_x++) {
            for (int32_t grid_c = 0; grid_c < kGridChannel; grid_c++) {
                ...
                if (box_confidence >= threshold_box_confidence_) {
                    ...
                    // Check if the detected class is the 'person' class
                    if (class_id == PERSON_CLASS_ID && confidence >= 0.10) {
                        int32_t cx = static_cast<int32_t>((data[index + 0] + 0) * scale_x); // cx calculation
                        int32_t cy = static_cast<int32_t>((data[index + 1] + 0) * scale_y); // cy calculation
                        int32_t w = static_cast<int32_t>(data[index + 2] * scale_x);        // width calculation
                        int32_t h = static_cast<int32_t>(data[index + 3] * scale_y);        // height calculation
                        int32_t x = cx - w / 2;
                        int32_t y = cy - h / 2;
